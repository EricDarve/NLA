...
The 2nd order Chebyshev iterative method is developed to eliminate the disadvantages of the 1st order method—namely, the requirement to choose the total number of iteration steps $k$ beforehand and the potential numerical instability. The two-step version is stable across iterations when the eigenvalue bounds $[a,b]$ are valid.

This method utilizes a two-step recurrence relation:

$$x^{(k+1)} = \alpha_k x^{(k)} + (1 - \alpha_k) x^{(k-1)} - \beta_k r^{(k)}, \quad k= 1, 2, \dots$$

where the iteration step starts with a special first step:

$$x^{(1)} = x^{(0)} - \frac{1}{2} \beta_0 r^{(0)}.$$

$r^{(k)}$ is the residual:

$$r^{(k)} = Ax^{(k)} - b$$

The construction relates the error polynomials $P_k(A)$ generated by this recurrence to the standard three-term recurrence relation of the Chebyshev polynomials of the first kind:
...
Let $a$ and $b$ be the lower and upper bounds, respectively, of the eigenvalues $\lambda_j$ of $A$, such that $0 < a < \lambda_j < b$. The optimal parameters are defined as, $k \ge 1$:
...
with starting parameter

$$\beta_0 = \frac{4}{a+b}$$

The primary advantage of this second-order method is that **it gives the optimal result for every step $k$** (apart from rounding errors), eliminating the need to choose the total number of iterations $k$ a priori.
...
| :--- | :--- | :--- |
| **Recurrence Relation** | Uses a one-step recurrence relation: $x^{k+1} = x^k - \tau_k(Ax^k - b)$. | Uses a two-step recurrence relation: $x^{k+1} = \alpha_k x^k + (1 - \alpha_k) x^{k-1} - \beta_k r^k$. |
| **Primary Pros** | Parameters are computed from a **closed expression**—that is, the one-step method's parameters are explicit functions of $a$, $b$, and $k$. Shares the **optimal asymptotic rate of convergence** (proportional to the square root of the condition number $O(\sqrt{\kappa})$). | **Numerically stable**. Gives the **optimal result for every step $k$**, apart from rounding errors, unlike the one-step method which is only optimized for a pre-chosen final step $k$. Eliminates the requirement to choose the number of iteration steps $k$ *a priori*. |
| **Primary Cons** | Major disadvantage is potential **numerical instability**. Instability occurs because, for certain iteration indices $k$, the iteration matrices can have a spectral radius much larger than 1. Requires the user to choose the total number of iteration steps $k$ **a priori**. The iteration parameters $\tau_k$ must be selected and used in a specific order (large values balanced by small values) to mitigate instability. | Parameters are derived from **recursions**. Requires more memory and complexity due to the two-step dependence. |
